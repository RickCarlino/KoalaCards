...
- 단, diagnosis.error_explanation 및 diagnosis.rules는 영어로 작성한다(교사 메모 용도). 이때 반드시 학습자에게 직접 말하듯 2인칭 "you"로 서술할 것(예: "You said …", "Use … instead"). "the learner", "the student" 같은 3인칭 표현은 금지. 수동태 남용 금지.
...
- 중복 금지: 각 섹션마다 동사/명사를 다양화(섹션별 최소 ${FLOOD_ITEM_COUNT_MIN}개 이상의 서로 다른 동사·명사).

사용 맥락(중요):
- flood.target / flood.contrast의 items[].text는 그대로 학습 카드와 음성(TTS)에 사용된다.
- 따라서 items는 "문장(text)"과 그 영어 풀이("en")만 포함한다. 부가 설명/주석 금지.

유효성 규칙:
- flood.target: label에 제시된 목표 형태를 모든 문장이 분명히 드러내야 한다(문법적이어야 함).
- flood.contrast: 대비 형태를 분명히 드러내되, target 형태는 절대 사용하지 않는다. 명확한 대비가 없으면 null.
...
1) diagnosis
  ...
  - rules: 영어 규칙 ${RULES_COUNT_MIN}–${RULES_COUNT_MAX}개(불릿)
2) flood
  - target: label + 예문 ${FLOOD_ITEM_COUNT_MIN}–${FLOOD_ITEM_COUNT_MAX}개(items[].text는 한국어, items[].en은 영어 풀이)
  - contrast: label + 예문 ${FLOOD_ITEM_COUNT_MIN}–${FLOOD_ITEM_COUNT_MAX}개 또는 null
3) production
  - ${INPUT_FLOOD_PRODUCTION_MIN}–${INPUT_FLOOD_PRODUCTION_MAX}개
  - 각 항목: prompt_en(영어 질문) + answer(한국어 정답). 한국어는 번역투 없이 자연스러운 표현으로.
...
- flood.contrast.items = ${FLOOD_ITEM_COUNT_MIN}–${FLOOD_ITEM_COUNT_MAX} 또는 null
