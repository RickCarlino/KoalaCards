...
const ImportPage: React.FC<ImportPageProps> = ({}) => {
  ...
  const handleSubmit = async () => {
    ...
    for (let line of lines) {
      ...
      if (!definition) {
        setError("A definition or example context is required after a tab character.");
        setIsLoading(false);
        return;
      }
    ...
    <Grid>
      <Col style={{ maxWidth: 600 }}>
        <Paper>
          ...
          <p>
            Enter a list of phrases below, one per line. Each line should
            contain a Korean word, followed by a tab character, followed by an
            either an English definition or a Korean sentence that contains the
            word.
          </p>
